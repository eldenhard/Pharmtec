...
const toggleVerifiedField = (item) => {
  const field =
    props.typeConfirm === "Кадровая служба"
      ? "is_verified_by_hr"
  ...
  const index = all_confirm_rows.value.findIndex(
    (row) => row.id === updatedItem.id
  );
...
watch(
  ...
  async () => {
    useLoaderStore().setLoader(true);
    try {
      invites.value = await api.getAllInvites(); // Перезагружаем данные при изменении вкладки
    } catch (err) {
      console.log("Ошибка при обновлении данных после смены вкладки:", err);
    } finally {
      useLoaderStore().setLoader(false);
    }
  }
...
const saveChanges = async () => {
  ...
  for (const row of all_confirm_rows.value) {
    try {
      ...
      formData.append("is_verified_by_hr", row.is_verified_by_hr);
      formData.append("is_verified_by_accounts", row.is_verified_by_accounts);

      if (row.is_verified_by_accounts && row.is_verified_by_hr) {
        await refreshToken();
        await api.editInvite(row.id, formData);
        await api.confirmNewInvite(row.id);
        ...
        if (props.typeConfirm === "Кадровая служба") {
          toast.success(
            `Подтверждение сотрудника кадровой службой подтверждено`,
            {
              timeout: 3000,
            }
          );
        } else {
          toast.success(`Подтверждение сотрудника бухгалтерией подтверждено`, {
            timeout: 3000,
          });
        }
      }
      ...
      toast.error(`Ошибка подтверждения сотрудника ${err}`, {
            timeout: 3000,
          });
      useLoaderStore().setLoader(false);
    }
  ...
  // Обновляем данные после завершения всех изменений
  try {
    await refreshToken();
    invites.value = await api.getAllInvites(); // Обновляем данные с сервера
  } catch (err) {
    console.log("Ошибка при обновлении данных после сохранения:", err);
  }
